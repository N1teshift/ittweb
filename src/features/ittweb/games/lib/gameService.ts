...
import type { Game, GamePlayer, GameWithPlayers, CreateGame, UpdateGame, GameFilters, GameListResponse } from '../types';
 ...
 */
function convertGameDoc(docData: any, id: string): Game {
  return {
    id,
    gameId: docData.gameId,
    datetime: timestampToIso(docData.datetime),
    ...
    replayUrl: docData.replayUrl,
    replayFileName: docData.replayFileName,
    submittedBy: docData.submittedBy,
    submittedAt: timestampToIso(docData.submittedAt),
 ...
 */
function convertGamePlayerDoc(docData: any, id: string): GamePlayer {
  return {
    id,
    gameId: docData.gameId,
    name: docData.name,
    ...
    flag: docData.flag,
    category: docData.category,
    elochange: docData.elochange,
    eloBefore: docData.eloBefore,
    eloAfter: docData.eloAfter,
    class: docData.class,
    randomClass: docData.randomClass,
    kills: docData.kills,
    deaths: docData.deaths,
    assists: docData.assists,
    gold: docData.gold,
    damageDealt: docData.damageDealt,
    damageTaken: docData.damageTaken,
    createdAt: timestampToIso(docData.createdAt),
  };
}
...
export async function getGames(filters: GameFilters = {}): Promise<GameListResponse> {
  try {
    ...
    if (isServerSide()) {
      const adminDb = getFirestoreAdmin();
      let gamesQuery = adminDb.collection(GAMES_COLLECTION) as any;
