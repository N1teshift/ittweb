...
import { uploadImage, extractYouTubeId, createArchiveEntry } from '@/lib/archiveService';
import { CreateArchiveEntry, DateInfo } from '@/types/archive';
...
export default function ArchiveForm({ onSuccess, onCancel }: ArchiveFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to create archive entry');
    } finally {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-gray-900 border border-amber-500/30 rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <h2 className="font-medieval-brand text-3xl mb-6 text-center">Add to Archives</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-4 pt-4">
            ...
            >
              {isSubmitting ? 'Adding...' : 'Add to Archives'}
            </button>
          </div>
        </form>
      </div>
