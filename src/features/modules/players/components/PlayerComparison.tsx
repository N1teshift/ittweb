...
export function PlayerComparison({ pageNamespaces: _pageNamespaces }: PlayerComparisonProps) {
  ...
  useEffect(() => {
    ...
    if (namesParam) {
      setPlayerNames(namesParam);
      fetchComparison(namesParam);
    } else {
      setLoading(false);
    }
  }, [router.query.names]);

  const fetchComparison = async (names: string) => {
    try {
      setLoading(true);
      setError(null);
  ...
  if (!comparison || !router.query.names) {
    return (
      <div className="min-h-[calc(100vh-8rem)]">
        <PageHero title="Compare Players" description="Compare statistics between players" />
        <div className="container mx-auto px-4 py-8">
          <Card variant="medieval" className="p-8">
            <form onSubmit={handleCompare} className="space-y-4">
              <div>
                <label htmlFor="names" className="block text-sm font-medium text-amber-400 mb-2">
                ...
                <p className="mt-2 text-sm text-gray-400">
                  Enter at least 2 player names separated by commas
                </p>
              </div>
              <button
                type="submit"
  ...
  const { players, headToHead, eloComparison } = comparison;
        ...
        {Object.keys(headToHead).length > 0 && (
          <Card variant="medieval" className="p-6">
            ...
            <div className="space-y-4">
              {Object.entries(headToHead).map(([player1, opponents]) => (
