...
jest.mock('@/features/modules/guides/utils/iconMap', () => ({
  ICON_MAP: {
...
jest.mock('@/features/modules/guides/data/abilities', () => ({ ABILITIES: [{ name: 'Chain Lightning' }] }));
jest.mock('@/features/modules/guides/data/units', () => ({
  BASE_TROLL_CLASSES: [{ name: 'Warrior' }],
  DERIVED_CLASSES: [{ name: 'Berserker' }],
}));
jest.mock('@/features/modules/guides/hooks/useItemsData', () => ({
  useItemsData: jest.fn(),
}));

const mockUseItemsData = jest.requireMock('@/features/modules/guides/hooks/useItemsData').useItemsData as jest.Mock;
