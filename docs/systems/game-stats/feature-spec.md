...
| Player Statistics | /players, /players/[name] | GET /api/players, GET /api/players/[name] | Shows lifetime + category stats, recent games, charts |
...
| Analytics & Charts | Player profile widgets, optional home highlights | /api/players (with includeGames, ctivity flags) | Activity area chart, ELO line chart, win-rate pie, class stats chart |
...
- GameList, GameCard, GameDetail – share loading/error/empty states.
- PlayerProfile, PlayerStats, PlayerCharts, PlayerGames – compose profile view.
...
- ClassOverview / ClassDetail – reuse card layout from guides to stay consistent.
...
- Build Firestore queries through uildGameQuery(filters) to centralize validation and index usage.
