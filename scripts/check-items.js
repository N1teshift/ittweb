// Simple extractor and comparator for ITT items
...
function extractItemsFromWurst(content, filePath) {
  /** @type {{ codeId?: string, name?: string, icon?: string, file: string }[]} */
  const items = [];
  ...
  const customDefRegex = /new\s+CustomItemDefinition\s*\(\s*([A-Z0-9_]+)[^)]*\)\s*([\s\S]*?)\)/g;
...
function extractExternalItems() {
  ...
  for (const f of files) {
    ...
    if (!txt) continue;
    // Quick check to skip non-item heavy files
  ...
  /** @type {Map<string, { codeId?: string, name: string, icon?: string, files: string[] }>} */
