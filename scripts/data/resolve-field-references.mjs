...
import { TMP_RAW_DIR, ABILITIES_TS_DIR, ITEMS_TS_DIR, UNITS_TS_DIR } from './paths.mjs';
  ...
  if (fs.existsSync(ABILITIES_TS_DIR)) {
    ...
    for (const file of files) {
      if (file.endsWith('.ts') && file !== 'index.ts' && file !== 'types.ts') {
        abilityFiles.push(path.join(ABILITIES_TS_DIR, file));
      }
    }
  }
...
function loadTypeScriptItems() {
  ...
  if (fs.existsSync(ITEMS_TS_DIR)) {
    const files = fs.readdirSync(ITEMS_TS_DIR, { recursive: true });
    for (const file of files) {
      if (file.endsWith('.ts') && file !== 'index.ts' && file !== 'types.ts' && !file.includes('abilityIdMapper')) {
        itemFiles.push(path.join(ITEMS_TS_DIR, file));
      }
    }
  }
...
function loadTypeScriptUnits() {
  ...
  if (fs.existsSync(UNITS_TS_DIR)) {
    const files = fs.readdirSync(UNITS_TS_DIR, { recursive: true });
    for (const file of files) {
      if (file.endsWith('.ts') && file !== 'index.ts' && file !== 'types.ts') {
        unitFiles.push(path.join(UNITS_TS_DIR, file));
      }
    }
  }
