...
function main() {
  ...
  console.log(`   Unique missing icon files: ${uniqueMissingIcons.size}`);
  if (uniqueMissingIcons.size > 0) {
    console.log(`\n   Missing icon files:`);
  ...
  const reportFile = path.join(ROOT_DIR, 'missing-icons-report.txt');
  ...
  console.log(`\n📄 Detailed report written to: ${path.relative(ROOT_DIR, reportFile)}`);
